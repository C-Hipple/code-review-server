...
3b. **Forced on-demand plugins**: `ApplyChanges` (manager.go) fires the same hook a second time, with `PRUpdateOptions{ForceOnDemandPlugins: true}`, for every PR it wrote into a section whose config sets `ForceRunOnDemand`. It is separate from step 3 because section membership isn't known until the item writes land, well after the cache warm. The deferred (`OnlyOnDemand`) plugins then run as ordinary automatic runs, keeping their per-SHA skip; `server.OnDemandPluginsPending` short-circuits the dispatch when nothing is outstanding, which is the steady state
