...
func TestApplyCacheWarmRequirements(t *testing.T) {
	...
	t.Run("warms every field GetPRDetails reads", func(t *testing.T) {
		...
		// ReviewThreads and Reactions have no filter that would ever request
		// them, so the warm is their only chance to land before someone opens
		// the review. Teams is the same story for draft and closed PRs, which
		// the open-PR rule skips.
		want := AuxDataRequirement{
			Diff: true, Commits: true, Reviews: true, Comments: true,
			ReviewThreads: true, Teams: true, Reactions: true,
		}
		if got != want {
