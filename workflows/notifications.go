...
func notifyPRAdded(sectionName, prTitle string) {
	switch runtime.GOOS {
	case "darwin":
		...
		if err := exec.Command("osascript", "-e", script).Run(); err != nil {
			slog.Warn("Failed to send macOS notification", "error", err)
		}
	default:
		slog.Error("DesktopNotifications is enabled but is not supported on this OS", "os", runtime.GOOS)
	}
}
